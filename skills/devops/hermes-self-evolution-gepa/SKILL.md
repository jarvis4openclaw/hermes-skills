...
43. **hermes-agent push to jarvis4openclaw always fails — skip it** — The `jarvis4openclaw/hermes-agent` fork does not exist on GitHub (the local repo at `~/.hermes/hermes-agent/` is a fork of `NousResearch/hermes-agent` with a `jarvis4openclaw` remote that points to a non-existent repository). Every cycle hits this: `gh auth switch`, remote URL surgery, "Permission denied" / "Repository not found" errors — all wasted work because the remote repo was never created. **Action:** do NOT attempt to push the hermes-agent repo at all. Just commit locally on the cycle branch and move on. The self-evolution and hermes-skills repos are the canonical push targets; the hermes-agent repo is local-only. Observed in Cycle 28 and traces back to at least Cycle 17.

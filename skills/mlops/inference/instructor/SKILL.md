...
- Prompt optimization → use `dspy` instead
- Building complex multi-stage AI pipelines → use `dspy` or LangChain
- When you don't need structured output → direct LLM calls are simpler
- Real-time chat applications → Instructor adds validation overhead per call
- When you need fine-grained control over the raw prompt → Instructor abstracts prompts

## Pitfalls

1. **Not setting `max_retries` explicitly.** The default is 3, but for production use you may want more or fewer. Set `max_retries` based on your tolerance for latency vs. accuracy.

2. **Using vague field descriptions.** The `Field(description="...")` text is sent to the LLM as guidance. Vague descriptions like "the data" produce inconsistent extractions. Be specific: "Product name as it appears in the text, without brand prefixes."

3. **Not using enums for constrained values.** If a field should only have specific values, use `Enum` instead of `str`. Without enums, the LLM may produce unexpected values that pass validation but are semantically wrong.

4. **Forgetting that `instructor.from_anthropic()` requires the `anthropic` package.** The base `instructor` package doesn't include provider bindings. Install `instructor[anthropic]` or `instructor[openai]`.

5. **Not handling `ValidationError` after retries exhausted.** If all retries fail, Instructor raises `pydantic.ValidationError`. Always wrap calls in try/except for production code.

6. **Using `Optional` fields without defaults.** `Optional[str]` without a default of `None` still requires the field in the JSON schema. Use `Optional[str] = None` to make it truly optional.

7. **Not using `mode=instructor.Mode.JSON` for providers without native tool calling.** Some local models don't support function calling. Use JSON mode as a fallback.

8. **Streaming partial objects without handling `None` fields.** When using `create_partial()`, fields may be `None` until the LLM generates them. Check for `None` before accessing attributes.

9. **Not providing examples in the prompt.** For complex schemas, include an example JSON in the user message. This dramatically improves extraction accuracy.

10. **Using `EmailStr` or `HttpUrl` without installing `pydantic[email]` or `pydantic[url]`.** These validators require extra dependencies. Install them or use custom `@field_validator` instead.

11. **Creating a new client per call.** Instructor clients are reusable. Create once and reuse — creating a new client per call adds unnecessary overhead.

12. **Not using `create_iterable` for list extraction.** If you want the LLM to generate a list of items, `create_iterable` is more reliable than asking for a JSON array in a single response.

...
metadata:
  hermes:
    ...
    trigger_conditions:
      - "Hermes Agent RL environment"
      - "Atropos training"
      - "build RL environment"
      - "agent loop reward function"
      - "HermesAgentBaseEnv"
      - "tool context reward"
      - "wandb logging RL"
      - "compute_reward"
      - "evaluate mode Atropos"
      - "serve mode Atropos"
      - "process mode Atropos"
      - "RL environment setup"
      - "reward verification tools"
---
...
- **Building new RL environments for Hermes Agent** — creating tasks where an agent loop calls tools and receives rewards
- **Designing reward functions for tool-using agents** — scoring agent rollouts based on tool execution, file outputs, or LLM judge evaluation
- **Setting up agent loop evaluation** — running `evaluate` mode with full multi-turn tool use against a held-out eval set
- **Configuring inference backends for RL training** — wiring OpenRouter, VLLM, or local Atropos servers to the environment
- **Logging RL metrics to wandb** — tracking mean reward, tool usage rates, and efficiency metrics during training
- **Debugging reward computation** — verifying that `compute_reward` correctly extracts final responses from `AgentResult.messages`

## Not For

- **General machine learning training** (non-RL) → use `axolotl` for supervised fine-tuning or `trl-fine-tuning` for standard RLHF/DPO
- **Single-turn LLM benchmarking** → use `lm-evaluation-harness` for static, non-agentic evaluation
- **Non-Hermes RL environments** → Atropos supports generic environments, but this skill is specific to the Hermes Agent tool-calling loop
- **Hyperparameter tuning** → use `weights-and-biases` sweeps or `grpo-rl-training` for optimization strategy guidance
- **Production deployment of trained models** → use `serving-llms-vllm` or `tensorrt-llm` for inference serving
...
1. **AgentResult has `.messages`, not `.final_response`** — Extract the final response by iterating `reversed(result.messages)` looking for the last assistant message with content. Do NOT access `result.final_response` — it does not exist.

2. **`evaluate()` must use `HermesAgentLoop`, not `chat_completion`** — Single-turn `chat_completion` has no tools. The whole point of hermes-agent benchmarks is agentic evaluation with tool use. Using `chat_completion` silently invalidates your evaluation.

3. **Don't call `_llm_judge` twice** — If `compute_reward` already calls it, extract the score from the buffer instead of calling the judge separately in `evaluate()`. Double-judging wastes inference budget and can produce inconsistent scores.

4. **Eval pollutes training buffers** — `compute_reward` appends to metric buffers. During evaluation, roll back buffer entries added during eval to keep training metrics clean. Otherwise your training curves show spikes on eval days.

5. **Always set `health_check=false` for OpenRouter** — OpenRouter has no `/health` endpoint. Atropos will hang waiting for a health check that never responds. Always pass `--openai.health_check false` with OpenRouter.

6. **Missing `data_dir_to_save_evals` in evaluate mode** — Without `--env.data_dir_to_save_evals`, evaluation results are computed but not persisted. You lose the eval samples and can't debug failures later.

7. **Confusing `default_toolsets` class variable with `enabled_toolsets` config** — The class variable `default_toolsets` is a hint; the config field `enabled_toolsets` is what actually controls tool resolution. Changing the class variable without updating config produces "tools not found" errors.

8. **Tool call parsing without `isinstance` guard** — Tool calls are dicts with `{"function": {"name": ..., "arguments": ...}}`. If you access `tc["function"]` assuming dict without checking `isinstance(tc, dict)`, you crash on unexpected formats. Always guard type access.

9. **Forgetting `ToolContext.cleanup()`** — `ToolContext` holds sandbox resources (temporary files, terminal sessions). Always call `ctx.cleanup()` in a `finally` block after `compute_reward`. Leaking sandboxes accumulates disk usage and port conflicts.

10. **Wrong `server_type` for external APIs** — Without `--openai.server_type openai`, Atropos assumes a local VLLM server and sends VLLM-specific parameters that external APIs reject. Set `server_type="openai"` for OpenRouter and any OpenAI-compatible endpoint.

11. **Hardcoding inference setup without asking the user** — Never assume OpenRouter or any specific model. The skill's Inference Setup section exists because provider choice, model choice, and endpoint URL vary by user and by session. Hardcoding destroys portability.

12. **Reward range drift** — `compute_reward` should return scores in `[0, 1]`. If rewards go outside this range or cluster at a single value (all 0.5), the RL optimizer can't learn. Always verify reward distribution with a histogram after a process-mode run.

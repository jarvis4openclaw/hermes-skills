...
- **Academic BibTeX / conference paper pipelines** — use \`research-paper-writing\` instead (this skill feeds it; see \`references/citation-formats.md\`).
- **Casual chat with no external facts** — skip inline citations when the retrieval is incidental (quick syntax lookup mid-coding, creative writing). Mention a URL only if the user would plausibly want the link.
- **Generating bibliographies from memory** — this skill requires ledger-registered sources; if you have no fetched sources, a plain reference list (or none) is honest.
- **Over-engineering short answers** — for a two-sentence reply with one source, inline the link rather than running the full ledger pipeline.
...
1. **Registering after writing** — The ledger must be populated from tool output,
  not reconstructed from the draft — that reintroduces exactly the hallucinated
  -URL risk the numbering removes. Recovery: reset and re-run retrieval → `add` → draft.
2. **Renumbering mid-task** — Never hand-edit ids in a draft. Ids are ledger
  identities; if a draft cites `[4]`, `[4]` must stay that source. Run `reset`
  only between tasks. Recovery: if ids drift, rebuild from the ledger rather than patching the draft.
3. **Retyping URLs into the Sources block** — Always `render`. A hand-typed URL
  is an unverified claim. Recovery: use `sources.py render --replace-in <draft>` and re-verify.
4. **Citing a search snippet as if you read the page** — A `web_search`
  description supports only what it literally says. Cite the extracted page
  when the claim needs the body — `web_extract` it first. Recovery: extract the page, add it, re-cite.
5. **Over-citing** — Three ids on a sentence is the ceiling; a citation on every
  clause makes text unreadable and hides which source carries the load. Recovery: merge ids per sentence, keep max 3.
6. **Citing the ledger in code/config artifacts** — Source comments belong in
  prose deliverables and doc headers, not inside generated code. Recovery: strip citations from code blocks before shipping.
7. **Parallel subagents** — Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `HERMES_CITATION_LEDGER`) if their
  outputs get merged, otherwise their ids will collide. Recovery: pass `--ledger` explicitly to every subagent.
8. **Quoting from a snippet instead of the page** — Evidence quotes must come
  from the extracted page text, not a search-result description — `web_extract`
  first, save the text, then `quote --from` that file. Recovery: extract, save, re-quote from the file.
9. **Paraphrasing into `quote --text`** — The verbatim check will reject it; the
  fix is to find the actual sentence, not to reword until something matches. Recovery: copy-paste the exact sentence from the fetched text.
10. **Using `[unverified]` as an escape hatch** — It marks the rare claim that
  genuinely cannot be sourced; if most sentences carry it, the task needed more
  retrieval, not more markers. Recovery: run additional retrieval for unverified claims before delivery.
11. **Hand-editing the Sources block** — Use `render --replace-in <draft>`; slicing
  the file yourself risks a stale or duplicated block that `verify` then flags. Recovery: re-render with `--replace-in` (idempotent) and re-run verify.
12. **Quoting markdown markup instead of visible prose** — web_extract returns
  inline links like `_[ERAP1](https://…)` and escaped asterisks (`HLA-B\*27`).
  Quote the sentence as the reader sees it — the matcher canonicalizes links to
  their label and drops emphasis/code/escapes on both sides, so reproducing the
  markup is both unnecessary and fragile. Recovery: paste the plain prose sentence.

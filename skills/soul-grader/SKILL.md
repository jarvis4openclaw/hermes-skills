...
metadata:
  hermes:
    ...
    trigger_conditions:
      ...
      - "score a soul file"
      - "audit soul.md"
      - "rewrite my agent identity"
      - "improve my soul"
      - "is my soul deployable"
...
- **Installing or configuring Hermes itself** — load `hermes-agent` skill instead.
- **Full new-agent intake/design** — load the project’s new-agent intake or design skill too, if one is available.
- **Authoring a new skill** — load `hermes-agent-skill-authoring` skill instead.
- **Editing a user’s actual SOUL file without permission** — always confirm before modifying.
- **Grading non-SOUL files (CLAUDE.md, AGENTS.md)** — those are different document types with their own conventions.
- **Generic prompt-engineering advice** — this skill uses its own bundled reference artifacts as the only grading standard.
...
1. **Grading from vibes** — Use the bundled references (soul-md-grading-standard.md, soul-md-field-guide.html, soul-md-wording-verbiage-layer.md), not generic prompt-engineering advice. If the bundled references are unavailable, stop and report it rather than grading from memory.
2. **Rewarding length over substance** — A long SOUL can still be weak if it lacks mission, negations, gates, and truth rules. Score against the rubric criteria, not word count.
3. **Punishing missing runbooks** — SOUL.md should NOT contain every command or workflow. Missing commands may belong in `CLAUDE.md`, `AGENTS.md`, skills, or manifests. Flag them as artifact separation issues, not content gaps.
4. **Ignoring agent scope** — Client/business SOUL files need stronger isolation, approval, credential, and handoff language than personal agents. Always classify scope first (personal, business/internal, client/business, public, meta, multi-agent, tactical) and adjust the safety bar accordingly.
5. **Treating YAML frontmatter as hidden** — Hermes native SOUL loading injects the entire SOUL content as prompt text. Frontmatter is visible unless an adapter strips/uses it before Hermes sees it. Secrets in frontmatter are an automatic fail.
6. **Forgetting session cache** — A corrected SOUL may not affect an existing Hermes session until a new session, restart, or compression rebuild. Note this when reporting deployability status for live agents.
7. **Over-rewriting personality** — Keep distinctive voice only when it serves mission and does not leak into public/client contexts improperly. Don't strip all character from a working identity just to hit rubric points.
8. **Missing false-claim thresholds** — Truth policy must name the specific tempting claims the agent could make (access, deployment state, health, publication, authority) and what verification proves each one. Generic "don't hallucinate" language is insufficient.
9. **Citing unapproved sources** — The bundled references are the only normative grading sources. Web search results, model-provider docs, and personal taste are not allowed as grading evidence unless the user explicitly requests a custom overlay after the source-grounded grade.
10. **Substituting a grade for a rewrite without permission** — Always report the grade first. Only provide replacement sections or a full revised SOUL when explicitly asked. A grade report with drift risks and suggested wording is the default deliverable.
11. **Missing automatic fail checks** — Always scan for secrets, false claims, ungated external actions, cross-client contamination, and frontmatter secrets BEFORE numeric scoring. An automatic fail makes the SOUL not deployable regardless of its numeric score.
12. **Conflating SOUL.md with companion files** — If a command or configuration belongs in `CLAUDE.md`, `AGENTS.md`, skills, or manifests, mark it as artifact separation, not as a SOUL defect. Do not reward or punish the SOUL for content that belongs elsewhere.

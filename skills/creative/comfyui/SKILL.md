...
- User wants to install custom nodes, manage the queue, or check available models
- User needs to convert an editor-format workflow template to API format

## Not For

- **Quick one-off image generation without workflow control** → use `comfyui`'s sibling skill `stable-diffusion` (if present) or a hosted generator — this skill assumes a running ComfyUI instance
- **Designing the look of a web page or UI** → use `frontend-taste` or `claude-design` instead
- **Audio generation with a dedicated tool** → use `kittentts-server` or songwriting/audio skills instead of ComfyUI's AudioCraft unless the workflow is already built
- **3D modelling outside ComfyUI nodes** → this skill covers ComfyUI node graphs only

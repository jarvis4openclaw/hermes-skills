...
metadata:
  hermes:
    ...
    trigger_conditions:
      - "remove my data from"
      - "opt me out"
      - "delete me from spokeo"
      - "delete me from whitepages"
      - "data broker"
      - "people search sites"
      - "clean up after a doxxing"
      - "remove my info from the internet"
      - "privacy cleanup"
      - "data removal"
      - "opt out of data brokers"
      - "remove my family from"
      - "remove my address from"
      - "recurring privacy monitoring"
---
...
- **Removing public records (voter/property/court) or accounts the person controls** → out of scope by design; record `human_task_queued` and move on
- **Defeating CAPTCHAs or anti-bot systems** → the skill never bypasses them; hard ones are queued for the operator browser
- **Acting on anyone without recorded consent** → never; intake is the single consent point
- **General OSINT/investigation of a person** → use `osint-investigation` instead (this skill is opt-out-focused)
- **Email/SMTP infrastructure debugging** → use `agentmail` instead (unbroker only sends via `$PDD send-email`)
...
## Pitfalls
1. **Never disclose more than the broker already shows.** Submit only `disclosure_fields`. The engine
  never volunteers SSN/ID numbers; you must not either.
2. **No consent, no action.** The engine enforces this; do not work around it to "research" a third party.
3. **`send-email` is idempotent + rate-limited.** It refuses to re-send a case already `submitted`
...
4. **Ledger writes are locked.** Concurrent runs (cron + manual) serialize safely; if you ever see a
  lock timeout, another run is mid-write - let it finish, don't delete the `.lock` by hand.
5. **Autonomy ≠ improvisation.** Full autonomy means not *asking* between steps; it does not loosen any
...
6. **Don't interrupt the run with questions.** Config choices are `setup --auto`'s job; human-only work
...
7. **Use `terminal`, not `execute_code`** for `pdd.py` (secret scrubbing + output redaction break it).
8. **Dossiers are plaintext by default** (JSON, `0600` under `HERMES_HOME`). For at-rest encryption run
...
9. **"Hidden from free search" ≠ deleted.** Only mark `confirmed_removed` after verifying the record is
  actually gone; note paid-tier retention in the report.
10. **Soft CAPTCHAs clear by default; don't fight the hard ones.** The default cloud browser passes
...
11. **Broker pages change.** If a flow breaks, `$PDD record ... blocked` and flag the broker file in
  `references/brokers/` for re-verification instead of guessing.
12. **Verify non-field-verified records before submitting.** `confidence: auto` records came from

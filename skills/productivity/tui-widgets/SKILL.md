...
metadata:
  hermes:
    ...
    trigger_conditions:
      ...
      - "add a ticker to the tui"
      - "live clock widget"
      - "status card widget"
      - "ambient panel hermes"
      - "modal overlay tui"
      - "widget dock"
      - "sparkline chart hermes"
      - "gauge bar hermes"
      - "react widget for hermes"
      - "tui widget app"
      - "add a panel to the terminal"
---
...
- **Desktop app widgets or GUI plugins** → use `hermes-desktop-plugins` instead.
- **Web dashboard panels** → use `hermes-webui-setup` or `automation-dashboard-update` instead.
- **CLI-only UX changes (no TUI)** — widgets require `hermes --tui`.
- **Full application development** — widgets are small single-purpose panels, not apps.
- **Hermes core feature development** → use `hermes-core-architecture` instead.
- **Notification or alert systems** — widgets render visually; they don't push notifications.

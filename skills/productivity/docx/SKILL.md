...
- **PDF documents** → use `pdf` skill instead.
- **Spreadsheets (.xlsx, .csv)** → use `xlsx` skill instead.
- **Presentations (.pptx)** → use `powerpoint` skill instead.
- **Scanned document OCR** → use `ocr-and-documents` skill instead.
- **Markdown or plain-text documents** — use `write_file` directly; docx is overkill for plain text.
- **Google Docs** → use `google-workspace` skill instead.
- **Batch document generation pipelines** — this skill handles one document at a time.

...
metadata:
  hermes:
    ...
    related_skills: [delegate-task-output-patterns, kanban-orchestrator, kanban-worker]
    trigger_conditions:
      - "fan out subagents"
      - "run parallel agents"
      - "codebase-wide sweep"
      ...
      - "refute claims"
      - "plan in code"
      - "too big for one context window"
      - "multi-angle research"
      - "hundreds of files"
      - "independent attempts cross-check"
when_to_use:
...
- Codebase-wide bug/security sweeps, 100+ file migrations, multi-angle research with sources cross-checked
- Any task too big for one context window where the split strategy is known enough to script (per-file, per-endpoint, per-source, per-record)
- High-stakes findings where you want independent attempts cross-checked / refuted before trusting the answer
- Orchestration that should be re-runnable as a script, not improvised turn-by-turn

## Not For

- **Small bounded tasks (<~10 units)** — just call the tool directly or do it inline; orchestration overhead is wasted
- **Tight serial dependency chains** (B needs A's output) — parallelism produces wrong/empty results; see the pitfalls
- **Durable work that must survive the user sending a new message or `/new`** — `delegate_task` is turn-scoped; use the kanban swarm (`kanban-orchestrator` / `kanban-worker`) or `cronjob` instead
- **Quick single-shot delegation** — for a handful of independent tasks with no verification recipe, plain `delegate_task` batch mode is enough
...
1. **Writing `delegate_task` inside an `execute_code` script** — It's not in `SANDBOX_ALLOWED_TOOLS`; the import/stub won't exist. Layer A is deterministic tools only. Fan out LLM judgment from the parent turn, not from inside a script.
2. **Promising background/resumable from `delegate_task`** — It's synchronous and turn-scoped. Durable = kanban swarm. If the user walks away mid-fan-out, every in-flight child is cancelled (status `interrupted`) and its work discarded — there is no cache-resume of a half-finished workflow.
3. **Trusting `summary` fields for content** — Route structured output to files (Pattern 2 in `delegate-task-output-patterns`). The `summary` field truncates under load.
4. **Non-atomic claims in the verify recipe** — Unfalsifiable claims survive refutation by default and pollute the output. Force located, specific claims ("endpoint `POST /api/users/:id/role` at src/routes/users.ts:142 has no role check"), never "the auth layer has problems."
5. **Same framing in all "independent" attempts** — They collapse to one answer and the cross-check is worthless. Vary the angle in each child's `context`.
6. **Fanning out a serial task** — If unit B needs unit A's output, parallelism produces wrong/empty results. Re-check independence before fanning out.
7. **Stale outputs from an interrupted run** — A prior interrupted run can leave outputs in a bare `/tmp/wf_<name>/` that look fresh. Always use a unique per-run directory `/tmp/wf_<name>_<uuid>/` and verify file count + freshness (`ls /tmp/wf_<name>/out_*.csv`) before synthesizing.
8. **Unbounded fan-out cost** — Each Layer-B child is a full agent tree (20 children ≈ 20× model calls); `delegation.max_concurrent_children` bounds concurrency only, not total. Always start on a SCOPED slice, prove the recipe end-to-end, report token cost, then offer full scale. Never silently fan out hundreds of children.

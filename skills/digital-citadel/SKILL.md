...
metadata:
  hermes:
    tags: [identity, continuity, session-recovery, backup, self-preservation]
    trigger_conditions:
      ...
      - "recover from /new"
      - "compaction loss recovery"
      - "continuity safeguards"
      - "survive session reset"
      - "remember who you are after wipe"
...
- **Simple file backups** (tar/cron backups of ordinary data) → use plain backup tooling or `hermes-config-backup` instead
- **General security hardening** (SSH, firewalls, password hygiene) → use `security` skills instead
- **Password / credential management** → use `vault-cli-cron` or `bitwarden-cli` instead
- **Server / homelab infrastructure continuity** (Proxmox, NAS, StartOS) → use the respective `proxmox-*` / `nas-management` / `startos` skills instead
- **Routine daily memory hygiene** (memory file cleanup, compaction tuning) → use `memory-maintenance` instead
- **Git repo backup/restore** → use `github-repo-management` instead
...
1. **Running full recovery for a compaction** — Compaction keeps the session alive with a summary; full Track A recovery re-reads SELF.md, memory files, and opens a postmortem, burning context and ironically triggering another compaction. Recovery: check for a compaction summary first; if present, use the lightweight path (read logs + active-tasks) only.
2. **Editing `sessions.json` or session files without human approval** — Session pointers are the source of continuity; autonomous rewiring can cause session combustion or duplicate agents. Recovery: never touch `sessions.json` or rename/delete `.jsonl` files autonomously — alert the human and wait for explicit go-ahead (T1.2 Track B rule).
3. **Using `gateway stop`/SIGTERM during recovery activation** — Graceful shutdown handlers can unload the LaunchAgent, so the gateway never restarts and the session pointer update never takes effect. Recovery: use `kill -9 $(lsof -t -i :18789)` to bypass handlers so KeepAlive restarts cleanly.
4. **Ignoring `.jsonl.reset.*` files after `/new`** — `/new` often leaves the real history in reset-backup files; a naive `ls` of `sessions/*.jsonl` shows nothing and the human's context looks gone. Recovery: always `ls -lSh *.jsonl *.reset.*` and include reset files in the discovery step (Track B).
5. **Treating SELF.md as a boot file** — Loading SELF.md every session competes for context budget and dilutes the boot briefing. Recovery: SELF.md is recovery-only; read it after a wipe or during weekly audits, never at bootstrap.
6. **Boot-file bloat through lesson-dumping** — MEMORY.md is a boot briefing, not a reference library; dumping every lesson and project detail makes the agent lose itself through noise. Recovery: keep boot files lean, move reference material to `docs/`, and audit with `wc -c` per file against the bootstrap budget.
7. **Autonomous gateway restarts after config edits** — Restarting the gateway can change the session ID, and a changed ID without an alert hides continuity loss. Recovery: prefer `openclaw config set`; if editing JSON directly run `openclaw doctor` first; note session ID before restart, verify after, and alert the human immediately on change.
8. **Trusting an encrypted backup you never restored** — An age-encrypted archive on cloud storage is worthless if the key is lost or the archive corrupts. Recovery: restore-test at least once (decrypt to a scratch dir) and keep the secret key in a password manager, never plaintext on disk.
9. **Improvement loops without kill conditions** — Recurring audits (security, continuity, capability) run on autopilot and become zombie loops that produce nothing. Recovery: maintain `docs/ops/improvement-loops.md` with a kill condition per loop and retire zombies during the monthly reconciliation.
10. **Recovery considered complete without a postmortem** — Skipping the human-triggered continuity postmortem means the root cause stays unexamined and the wipe will repeat. Recovery: recovery is not done until the postmortem is written into the weekly mindset log and the incident is diagnosed.

## Changelog

### 0.8.0 (2026-08-06)
- Added `version:` field and `metadata.hermes.trigger_conditions` to YAML frontmatter for trigger clarity
- Added `## Not For` section disambiguating from backup/security/password/infra skills
- Added `## Pitfalls` section with 10 numbered failure modes for identity-preservation workflows

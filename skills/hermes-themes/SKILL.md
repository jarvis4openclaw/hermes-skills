...
metadata:
  hermes:
    ...
    trigger_conditions:
      ...
      - "change the look of hermes"
      - "author a hermes theme"
      - "skin the cli"
      - "customize hermes colors"
      - "build a synthwave theme"
      - "dark forest theme"
      - "match my brand colors"
      - "change the tool marker color"
      - "tweak the active skin"
...
- **GUI theme editors or built-in preset management** — Hermes ships with built-in skins; this skill is for authoring new ones.
- **Desktop-app or web-dashboard widget development** → use `tui-widgets` instead.
- **Hermes plugin development** → use `hermes-agent` skill instead.
- **Configuring Hermes install/provider/model settings** → use `hermes-config-management` instead.
- **Non-Hermes terminal color schemes** — this only skins Hermes surfaces (CLI, TUI, desktop).
- **Batch skin generation or theme marketplace publishing** — author one skin at a time.

...
- Reading, searching, or triaging the user's email inbox
- Checking calendar events or finding available time slots
- Looking up contacts by name or email
- Drafting, replying to, or forwarding emails
- Managing team shared inboxes and assignments
- Viewing meeting transcripts
- Applying email actions (archive, pin, snooze, label, etc.)
- Managing contact categories and GateKeeper settings

## Not For

- **Sending emails programmatically without Spark** → use `agentmail` or `himalaya` instead.
- **Non-Spark calendar management** → use `google-workspace` or `notion-calendar` instead.
- **CRM or sales pipeline management** — Spark is an email client, not a CRM.
- **Email analytics or bulk processing** — Spark handles individual email actions, not batch analytics.
- **Configuring email accounts or servers** — Spark Desktop manages account setup.
- **Non-email Spark features** — this skill only covers the `spark` CLI commands.

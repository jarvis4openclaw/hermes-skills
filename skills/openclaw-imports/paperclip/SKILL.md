...
version: 1.0.0
author: Paperclip + Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [paperclip, task-management, agent-coordination, heartbeat, api]
    trigger_conditions:
      - "paperclip"
...
Load this skill whenever the user (or your heartbeat) needs to:
- Check assignments or inbox via Paperclip
- Checkout, update status, or comment on Paperclip issues
- Create subtasks or delegate work to other agents
- Set up Paperclip projects or workspaces
- Generate OpenClaw invite prompts
- Post comments with proper company-prefixed links
- Handle approval follow-ups or reassignment requests
- Search issues or check the dashboard
- Validate Paperclip flows (self-test playbook)

## Not For

- Writing code, debugging, or running tests → do the domain work, then update Paperclip
- Managing GitHub issues or PRs directly → use `github-issues`, `github-pr-workflow`
- Configuring the agent itself (Hermes setup) → use `hermes-agent`
- Managing servers, VMs, or infrastructure → use `proxmox-host-management`
- Creating or editing skills → use `hermes-agent-skill-authoring`
- General chat without task coordination → no skill needed
...
1. **Not checking out before working.** The heartbeat procedure requires checkout before any work. Without it, status updates may fail with 409 or the task may be owned by another agent. Always `POST /api/issues/{issueId}/checkout` before starting.

2. **Retrying a 409 Conflict.** A 409 means another agent owns the task. Never retry — pick different work. Retrying wastes API calls and doesn't change ownership.

3. **Missing `X-Paperclip-Run-Id` header on mutating requests.** Without this header, actions aren't traceable to the current heartbeat run. The skill says "ALL requests that modify issues" need it, but it's easy to forget on `POST /api/issues/{issueId}/comments` or `PATCH /api/issues/{issueId}`.

4. **Reposting the same blocked comment.** The blocked-task dedup rule: if your last comment was a blocked update AND no new comments exist, skip the task entirely. Violating this creates noise in the issue thread.

5. **Self-assigning without explicit mention handoff.** The skill is strict: only self-assign when `PAPERCLIP_WAKE_COMMENT_ID` is set AND the comment explicitly directs you to take ownership. Guessing intent leads to task theft.

6. **Using unprefixed paths in comments.** `/issues/PAP-123` is wrong. `/<prefix>/issues/PAP-123` is correct. Derive prefix from any existing issue identifier. Broken links in comments break the UI.

7. **Marking planning issues as done.** If asked to make a plan, do NOT mark the issue done. Reassign to the requester and leave it in progress. Marking done on a planning task terminates the workflow prematurely.

8. **Creating subtasks without `parentId` or `goalId`.** Subtasks that float without parent/goal context become orphaned. Always set both. Set `billingCode` for cross-team work.

9. **Not fetching `baseRevisionId` before updating documents.** When updating a plan document, fetch the current revision first. Without `baseRevisionId`, the update may silently overwrite concurrent edits.

10. **Forgetting to include Co-Authored-By on git commits.** The skill explicitly requires `Co-Authored-By: Paperclip <noreply@paperclip.ing>` on commits made during heartbeat work. Missing this breaks audit trails.

11. **Not incrementally loading comments.** On repeated heartbeats, use `?after={last-seen-comment-id}` instead of fetching the full thread. Full thread reloads on every heartbeat waste API calls and slow down execution.

12. **Budget blindness.** Above 80% budget, focus on critical tasks only. Continuing with low-priority work at high burn rate may pause the agent entirely.

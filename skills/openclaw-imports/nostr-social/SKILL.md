...
- You want to **give your agent a social identity** → use to create Nostr keys and profile
- You need **ecash wallet functionality** → use to create and manage Cashu wallet
- You want **agent-owned keys and sats** → use to establish agent sovereignty
- You're setting up **agent autonomy** → use to bootstrap identity and wallet
- You want **no third parties** → use to establish direct agent ownership
- You need **NIP-06 mnemonic backup** → use to create recoverable identity

## Not For

- **Managing your personal Nostr keys** → use your own Nostr client instead
- **General Bitcoin wallet operations** → use a dedicated Bitcoin wallet instead
- **Lightning network payments as a human** → use a human-focused Lightning wallet instead
- **Creating identities for non-agent entities** → use a general-purpose identity tool instead
- **Complex wallet operations requiring human approval** → use a human-managed system instead
...
1. **Missing user consent for key generation** — The setup flow requires explicit user consent before generating cryptographic keys. Recovery action: Always present the security notice and prompt for consent with `read -p "Continue with key generation? (yes/no): "`.

2. **Failing to back up the recovery phrase** — The mnemonic in `~/.cocod/config.json` must be backed up by the user. Recovery action: Emphasize the importance and wait for user confirmation with "done" before proceeding.

3. **Not resolving NIP-05 identifiers** — Owner's NIP-05 identifiers must be resolved to npub before following. Recovery action: Run `node {SKILL_DIR}/scripts/nostr.js lookup <nip05>` before `nostr.js follow`.

4. **Using shell injection in posting commands** — User input in posts must be handled safely. Recovery action: Use stdin for content with `echo "message" | nostr.js post -` to prevent injection.

5. **Not checking rate limits before autoresponses** — The autoresponse system has a maximum of 10 responses per hour. Recovery action: Always run `nostr.js rate-limit` before processing mentions.

6. **Forgetting to set the MINT environment variable** — The Cashu wallet requires a mint URL. Recovery action: Set `export MINT=https://mint.minibits.cash/Bitcoin` before wallet operations.

7. **Not handling WoT source configuration** — The autoresponse depends on the OWNER_PUBKEY being defined in `nostr.js`. Recovery action: Verify the configuration file contains the correct public key.

8. **Using non-deterministic profile images** — Profile images should be unique per npub but consistent across restarts. Recovery action: Use DiceBear with the npub as seed: `https://api.dicebear.com/7.x/shapes/png?seed=${NPUB}`.

9. **Not integrating with SOUL.md for voice consistency** — Posts should match the agent's personality. Recovery action: Pull name and about text from SOUL.md or IDENTITY.md.

10. **Skipping Heartbeat integration** — Nostr activity should be checked periodically. Recovery action: Add the mention and DM checks to the heartbeat rotation every 2-4 hours.

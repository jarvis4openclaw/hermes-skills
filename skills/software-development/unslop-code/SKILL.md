...
metadata:
  hermes:
    tags: [ai-slop, code-quality, audit, unslop]
    trigger_conditions:
      - "de-slop this code"
      - "this code looks AI-generated"
      - "reads like a tutorial"
      - "too generic / vibe-coded"
      - "make this code look human-written"
      - "audit code for AI tells"
      - "remove AI tells from this file"
      - "why does this code look AI"
      - "boilerplate / tutorial-shaped code"
...
- User asks to make generated code "not look AI" or "de-slop this" before shipping.
- Auditing a diff/repo for AI tells before commit or review.
- Writing new code where the surrounding codebase style must dominate over model defaults.
- Debugging why a reviewer said the code "reads like a tutorial" or "looks vibe-coded."
- Running the scanner (`scripts/unslop_code_scan.py`) in CI as a gate.

## Not For

- Writing the code for you from scratch → that is normal development, not this skill.
- Styling a website/UI that reads as AI → use `unslop-ui` instead.
- Rewriting prose to sound human → use `unslop-text` (or `no-ai-slop` / `humanizer`) instead.
- General code review for correctness/security → use `code-review` / `requesting-code-review` instead.
- Enforcing project style rules (linting/formatting) → use the project's linter/formatter instead; this skill complements, not replaces, them.
...
made-up API, over-engineering a simple task, and ignores how the rest of the repo works. Those
...
against real docs:
- Python: `python -m py_compile `, `ruff check` or `pyflakes`, `mypy`, and actually
...
```bash
python3 scripts/unslop_code_scan.py  # full report + slop score
python3 scripts/unslop_code_scan.py  --severity high # only the strongest signals
python3 scripts/unslop_code_scan.py  --json # machine-readable, for CI
```
...
## Respecting intentional choices.
A line containing `unslop-ignore` is skipped. Use it when a flagged construct is a real decision (a
broad catch at a boundary, an emoji in a CLI banner), so the audit stays trustworthy.
## Fixing well: the anchor is correct, and fits this codebase.
Do not "fix" `process_data()` by renaming it `processDataFunction()`, which is just a different tell. Name it for what it does, the way the rest of the repo names things. Code has far less aesthetic latitude than prose or a UI: there is rarely a "tasteful choice" to make here, only "is it correct, and does it match what is already in this project." A fix that swaps the model's default for your own invented default is not a fix; a fix that makes the line look like the code around it is.
...
1. **Polishing cosmetics while a structural tell ships** — Cleaning emoji and comments while leaving a swallowed `except`, a hallucinated API call, or a `// rest of your code` stub is the failure mode the data ranks loudest. Always run the code/build first (Mode 2 step 1) and fix bug-class tells regardless of severity, before touching cosmetics.

2. **Over-correcting into "trying to look senior"** — Told to make code clean, models add defensive checks for impossible cases, annotate every local, and comment every block — which is itself a tell. The anchor is the surrounding code: do not add a check, type, comment, or layer the neighboring code would not have.

3. **Trusting the scanner alone** — `scripts/unslop_code_scan.py` is a regex. It cannot see tutorial shape, over-engineering, or repo mismatch — the loudest tells in the data. Scanner clean ≠ clean code. Read the diff and check structural tells by eye (or with `references/tells.md`).

4. **Fixing a default with another default** — Renaming `process_data()` to `processDataFunction()` is just a different tell. Name things for what they do the way the rest of the repo names them; a fix that swaps the model's default for your own invented default is not a fix.

5. **Generating against a blank slate** — Skipping the brief produces the median of public code, i.e. the tutorial. Establish the surrounding code, the real requirement, and verify every import/call against real docs before writing (Mode 1).

6. **Flagging intentional choices** — A broad catch at a real boundary or an emoji in a CLI banner is a decision, not slop. A line containing `unslop-ignore` is skipped; use it so the audit stays trustworthy and you don't nag users into ignoring the tool.

7. **Fixing every flagged item regardless of class/severity** — The scanner reports class (bug vs cosmetic) and severity. Bug-class findings always get fixed; cosmetic findings are worth removing but nothing breaks if one survives. Spending an audit equalizing both wastes effort on the cheap 40%.

...
- Things `print()` / `logging.debug` solve in under a minute → use print debugging first
- Things `pytest -vv --tb=long --showlocals` already reveals → read the traceback
- Profiling or performance analysis → use `cProfile` or `py-spy`
- Debugging non-Python processes → use `node-inspect-debugger` or `debugging-hermes-tui-commands`
- Systematic root cause analysis of complex bugs → use `systematic-debugging` first
...
10. **Forgetting to remove debug instrumentation before committing.** Always run the cleanup grep before pushing:
    ```bash
    rg -n 'breakpoint\(\)|set_trace\(|debugpy\.listen' --type py
    ```

11. **Using `debugpy` when `remote-pdb` suffices.** For terminal-only debugging, `remote-pdb` is simpler — no DAP protocol, just `nc` to a port. Use `debugpy` only when you need IDE integration.

12. **Hardcoded paths in `pathMappings` for VS Code attach.** The `remoteRoot` must match the actual process working directory. Verify with `pwdx <pid>` before configuring.
